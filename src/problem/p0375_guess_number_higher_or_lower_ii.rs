...
impl Solution {
    pub fn helper(start : i32, end : i32, cache : &mut HashMap<(i32, i32), i32>) -> i32 {
        // start, end both inclusive
        if start >= end {
            0
        } else if let Some(&cached) = cache.get(&(start, end)) {
            cached
        } else {
            let mut min_cost : i32 = 1<<30;
            for guess in start..=end {
                let this_cost = guess + std::cmp::max(Self::helper(start, guess-1, cache), Self::helper(guess+1, end, cache));
                min_cost = std::cmp::min(min_cost, this_cost);
            }
            cache.insert((start, end), min_cost);
            min_cost
        }
    }
