...
impl Solution {
    pub fn helper(root: &Option<Rc<RefCell<TreeNode>>>, p: &Option<Rc<RefCell<TreeNode>>>, q: &Option<Rc<RefCell<TreeNode>>>) -> Option<Rc<RefCell<TreeNode>>> {
        let p_value = p.as_ref().unwrap().borrow().val;
        let q_value = q.as_ref().unwrap().borrow().val;
        let r_value = root.as_ref().unwrap().borrow().val;

        if r_value == p_value || r_value == q_value || (p_value < r_value && r_value < q_value){
            Some(Rc::new(RefCell::new(TreeNode::new(r_value))))
        } else if q_value < r_value {
            Self::helper(&root.as_ref().unwrap().borrow().left, p, q)
        } else if r_value < p_value {
            Self::helper(&root.as_ref().unwrap().borrow().right, p, q)
        } else {
            panic!("Not here")
        }
    }
